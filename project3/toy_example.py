...
def make_pair_of_players(X_, y_=None):
    ...
    for i in range(n_):
        ...
        time_start = X_.iloc[i].time_start
        ...
        for player_j in players:
            ...
            X_pairs.iloc[idx] = [sender,  p_i_["x_{:0.0f}".format(sender)], p_i_["y_{:0.0f}".format(sender)],
                                 player_j, p_i_["x_{:0.0f}".format(player_j)], p_i_["y_{:0.0f}".format(player_j)], time_start,same_team_(sender, player_j)]
...
def compute_distance_to_goal(X_):
    ...
    d = np.sqrt((X_["x_j"]-5250)**2 + (X_["y_j"])**2)
    return d
...
if __name__ == '__main__':
    ...
    X_LS_pairs["distance_to_goal"] = compute_distance_to_goal(X_LS_pairs)
    ...
    X_TS_pairs["distance_to_goal"] = compute_distance_to_goal(X_TS_pairs)
  
    X_TS_features = X_TS_pairs[["distance", "same_team","distance_to_goal","time_start"]]
