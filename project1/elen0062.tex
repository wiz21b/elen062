...
\subsection{Proof}
% That's probably bullshit. Well, I tried..
...
\begin{align*}
\sum\limits_{i=1}^N 2(\Delta_{k,i}y_i - w_k a_{k,i})(-a_{k,i}) = 0 \\
\sum\limits_{i=1}^N (\Delta_{k,i}y_i - w_k a_{k,i})a_{k,i} = 0  \\
\sum\limits_{i=1}^N \Delta_{k,i}y_i a_{k,i} - \sum\limits_{i=1}^N w_k a_{k,i} a_{k,i} = 0 \\
w_k \sum\limits_{i=1}^N  a_{k,i} a_{k,i} = \sum\limits_{i=1}^N \Delta_{k,i}y_i a_{k,i} \\
w_k = \frac{\sum\limits_{i=1}^N \Delta_{k,i}y_i a_{k,i}} {\sum\limits_{i=1}^N  a_{k,i} a_{k,i} }\\
w_k = \frac{N^{-1}\sum\limits_{i=1}^N \Delta_{k,i}y_i a_{k,i}}{N^{-1}\sum\limits_{i=1}^N  a_{k,i}^2}  \\
w_k = \frac{\Cov(\Delta_ky, a_k)}{N^{-1}\sum\limits_{i=1}^N  (\overline{a} - a_{k,i})^2} && \text{because prewhitening, } \overline{a} = 0 \\
w_k = \frac{\Cov(\Delta_ky, a_k)}{\sigma_{a_k}^2} && \text{because prewhitening, } \sigma_{a_k} = 1 \\
w_k = \Cov(\Delta_ky, a_k) \tag{eq. 1} \label{myeq}
\end{align*}

The proposed $w_k$ is $\rho_{\Delta_ky} \times {\Delta_k}y$ with $\rho$ the Pearson's coefficient.
\begin{align*}
w_k= \frac{\Cov(\Delta_ky, a_k)}{\sigma_{\Delta_ky} \sigma_{a_k}} \sigma_{\Delta_ky} && \text{simplifying } \sigma_{\Delta_ky} \\
w_k= \frac{\Cov(\Delta_ky, a_k)}{\sigma_{a_k}} && \text{because prewhitening, } \sigma_{a_k} = 1 \\
w_k= \Cov(\Delta_ky, a_k)
\end{align*}
which is equal to \ref{myeq}.
\subsection{Implement algorithm}
See code.
\subsection{Experiment 1}
	\begin{figure}[H]
	\centering
	\subfloat{\includegraphics[width=\picwidth]{plots/linreg/rl1a.pdf}}
	\subfloat{\includegraphics[width=\picwidth]{plots/linreg/rl1b.pdf}}
...
\subsection{Experiment 2}
	\begin{figure}[H]
	\centering
	\subfloat{\includegraphics[width=\picwidth]{plots/linreg/rl2.pdf}}
